...
public:
    DemoScene() {
        ...
        // suzane
        Instantiate<Suzane>(new Suzane(new Phong()), Vector3{ 6, 1.5, -3 });
        Instantiate<Suzane>(new Suzane(), Vector3{ 2, 1.5, -3 });
        Instantiate<Suzane>(new Suzane(new Diffuse()), Vector3{ -2, 1.5, -3 });
        Instantiate<Suzane>(new Suzane(new Diffuse(uvGrid)), Vector3{ -6, 1.5, -3 });
        Instantiate(new Suzane(new Normal()), Vector3{ 6, 1.5, -7 });
        Instantiate(new Suzane(new Unlit(uvGrid)), Vector3{ 2, 1.5, -7 });
        Instantiate(new Suzane(new Unlit()), Vector3{ -2, 1.5, -7 });
        Instantiate(new Suzane(new UV()), Vector3{ -6, 1.5, -7 });

        Material* multiLightMat = new Phong();
        Instantiate(new Primitive(PrimitiveType::SphereMesh, multiLightMat), Vector3{ 2, 1.5, 0 });
        Instantiate(new Suzane(multiLightMat), Vector3{ -2, 1.5, 0 });
    }
};

class SuzaneScene : public DefaultScene
{
public:
    SuzaneScene() {
        light = DirectionalLight(ColorRGB{ .7, .7, .7 }, Vector3{ 0, -.3, -.7 });
        defaultCam->Move(Vector3{ 3, 0, -3 });
        defaultCam->Rotate(Vector3{ 0, 35, 0 });

        // suzane
        Texture tex = ResourceManager::LoadTexture("CondorEngine/textures/UVGrid.png");
        Material* mat = new Phong();
        Instantiate<Suzane>(new Suzane(mat), Vector3{ 0, 1.5, -3 });
    }
};
